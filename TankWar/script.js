window.addEventListener('load', function () {
    ...
    class Electricty {
        constructor(game, x, y) {
            ...
            this.speedX = 20;
            let moveY = 0;
            const targetEnemy = this.game.enemies.find((enemy) =>
                badEnemyTypes.includes(enemy.type) && (enemy.x > this.x) && (enemy.x < this.game.width)
            )
            if (targetEnemy) {
                moveY = 20 * (targetEnemy.y - this.y) / (targetEnemy.x - this.x);
            }
            this.speedY = moveY;
            this.markedForDeletion = false;
    ...
    class Player {
        ...
        update(deltaTime) {
            ...
            if (this.game.keys.includes(' ') && this.game.shoottime <= 0) {
                this.game.player.shootTop();
                this.game.shoottime = 500;
    ...
    class Game {
        constructor(width, height) {
            ...
            this.helpers = [new Turtle(this, 0), new eel(this, 0), new Helper(this, 100), new Helper(this, 300), new Dolphin(this, 300)];
            this.freeze = false;
            this.smokegernade = 1;
            this.hide = false;
            this.hidetimer = 1500;
